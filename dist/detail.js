...
if (project && root) {
  ...
  root.innerHTML = `<article style="--case-accent:${accent}">
    ...
    <section class="case-section contribution-section"><div class="contribution-head"><div><p class="eyebrow">My contribution</p><h2>What I built and analyzed.</h2></div><p>${escapeText(project.analysisOutcome || project.summary)}</p></div>${list(project.built,'built-grid',(item)=>`<div class="built-item"><p>${escapeText(item)}</p></div>`)}<div class="method-strip"><span>Methods & tools</span><div class="tech-list">${project.tech.map((tech)=>`<span>${escapeText(tech)}</span>`).join('')}</div></div></section>
    <section class="case-section findings-section"><p class="eyebrow">Findings / Product Decision</p><h2>What the analysis changed.</h2>${list(project.results,'result-grid',(result,index)=>`<div class="result-card"><b>0${index+1}</b><p>${escapeText(result)}</p></div>`)}<div class="decision-callout"><span>Decision output</span><p>${escapeText(project.analysisOutcome || project.summary)}</p></div><div class="limits-grid"><div><p class="eyebrow">Evidence boundary</p><p>${escapeText(project.limitations || 'Results are presented within the scope of the available project evidence.')}</p></div><div><p class="eyebrow">What I learned</p><p>${escapeText(project.learned)}</p></div></div><div class="case-links">${links}<a class="button dark-outline" href="assets/Wenjing_Tian_Resume.pdf" download>Download résumé</a></div><a class="next-project" href="project.html?id=${next.id}"><span>Next case study</span><strong>${escapeText(next.title)} ↗</strong></a></section>
  </article>`;
}
